...
<h1 align="center">🦀 AndroidClaw — Osobní AI Asistent</h1>
...
androidclaw gateway                # výchozí: 127.0.0.1:42617
androidclaw gateway --port 0       # náhodný port (posílené zabezpečení)
...
Aktualizujete? Spusťte `androidclaw doctor` po aktualizaci.
...
AndroidClaw může importovat váš workspace, paměť a konfiguraci OpenClaw:
...
Migruje záznamy paměti, soubory workspace a konfiguraci z `~/.openclaw/` do `~/.androidclaw/`. Konfigurace je automaticky převedena z JSON do TOML.

## Výchozí nastavení zabezpečení (přístup DM)

AndroidClaw se připojuje k reálným komunikačním platformám. Zacházejte s příchozími DM jako s nedůvěryhodným vstupem.
...
- Spusťte `androidclaw doctor` pro odhalení rizikových nebo špatně nakonfigurovaných DM politik.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritické_  | **Nejsme spojeni** s `openagen/zeroclaw`, `zeroclaw.org` ani `zeroclaw.net`. Domény `zeroclaw.org` a `zeroclaw.net` aktuálně směřují na fork `openagen/zeroclaw` a tato doména/repozitář se vydávají za naši oficiální stránku/projekt.                                                                                       | Nedůvěřujte informacím, binárním souborům, sbírkám ani oznámením z těchto zdrojů. Používejte pouze [toto repozitárium](https://github.com/darkansem12-rgb/zeroclaw-android) a naše ověřené sociální účty.                                                                                                                                                                                                                                                                                                                                                                       |
| 2026-02-21 | _Důležité_ | Naše oficiální webové stránky jsou nyní aktivní: [](https://). Děkujeme za trpělivost při přípravě spuštění. Stále vidíme pokusy o vydávání se za nás, takže se **ne**připojujte k žádným investicím nebo sbírkám pod jménem AndroidClaw, pokud nebyly zveřejněny prostřednictvím našich oficiálních kanálů.                            | Používejte [toto repozitárium](https://github.com/darkansem12-rgb/zeroclaw-android) jako jediný zdroj pravdy. Sledujte [X ()](), [Facebook (Skupina)]() a [Reddit (r/zeroclawlabs)]() pro oficiální aktualizace. |
...
## Srovnání výkonu (AndroidClaw vs OpenClaw, reprodukovatelné)
...
> Poznámky: Výsledky AndroidClaw jsou měřeny na release buildech pomocí `/usr/bin/time -l`. OpenClaw vyžaduje běhové prostředí Node.js (typicky ~390MB dodatečné paměťové režie), zatímco NanoBot vyžaduje běhové prostředí Python. PicoClaw a AndroidClaw jsou statické binárky. Výše uvedené hodnoty RAM jsou runtime paměť; požadavky kompilace jsou vyšší.
...
Minimální `~/.androidclaw/config.toml`:
...
AndroidClaw podporuje nativní autorizační profily předplatného (více účtů, šifrování v klidu).

- Soubor úložiště: `~/.androidclaw/auth-profiles.json`
- Šifrovací klíč: `~/.androidclaw/.secret_key`
...
Kořenový adresář workspace: `~/.androidclaw/workspace/` (konfigurovatelné přes config).
...
Dovednosti: `~/.androidclaw/workspace/skills/<skill>/SKILL.md` nebo `SKILL.toml`.
...
# Gateway + démon
androidclaw gateway              # Spuštění gateway serveru (127.0.0.1:42617)
androidclaw daemon               # Spuštění plného autonomního runtime

# Agent
androidclaw agent                # Interaktivní režim chatu
androidclaw agent -m "message"   # Režim jedné zprávy
...
# Kanály
androidclaw channel list         # Seznam konfigurovaných kanálů
androidclaw channel doctor       # Kontrola zdraví kanálů
androidclaw channel bind-telegram 123456789

# Cron + plánování
androidclaw cron list            # Seznam naplánovaných úloh
...
androidclaw hardware discover    # Skenování připojených zařízení
androidclaw peripheral list      # Seznam připojených periferií
androidclaw peripheral flash     # Flash firmwaru na zařízení
...
Nebo přeskočte výše uvedené kroky a nainstalujte vše (systémové závislosti, Rust, AndroidClaw) jedním příkazem:
...
- Následujte [AndroidClaw Onboard](#rychlý-start) pro průvodce nastavením.
...
AndroidClaw byl vytvořen pro smooth crab 🦀, rychlého a efektivního AI asistenta. Vytvořil Argenis De La Rosa a komunita.

- [](https://)
- [](https://x.com/zeroclawlabs)

## Podpořte AndroidClaw

Pokud vám AndroidClaw pomáhá v práci a chcete podpořit další vývoj, můžete přispět zde:
...
Jste v AndroidClaw noví? Hledejte issues označené [`good first issue`](https://github.com/darkansem12-rgb/zeroclaw-android/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — podívejte se na náš [Průvodce přispíváním](CONTRIBUTING.md#first-time-contributors), jak začít. AI/vibe-coded PR vítány! 🤖
...
- Nový `Skill` → `~/.androidclaw/workspace/skills/<name>/`
...
**Toto je jediný oficiální repozitář AndroidClaw:**

> https://github.com/darkansem12-rgb/zeroclaw-android

Jakýkoli jiný repozitář, organizace, doména nebo balíček tvrdící, že je „AndroidClaw" nebo naznačující spojení se AndroidClaw Labs je **neautorizovaný a není spojen s tímto projektem**. Známé neautorizované forky budou uvedeny v [TRADEMARK.md](docs/maintainers/trademark.md).

Pokud narazíte na podvržení identity nebo zneužití ochranné známky, prosím [otevřete issue](https://github.com/darkansem12-rgb/zeroclaw-android/issues).

---

## Licence

AndroidClaw je dvojitě licencován pro maximální otevřenost a ochranu přispěvatelů:
...
Název **AndroidClaw** a logo jsou ochranné známky AndroidClaw Labs. Tato licence neuděluje povolení k jejich použití pro naznačení podpory nebo spojení. Viz [TRADEMARK.md](docs/maintainers/trademark.md) pro povolená a zakázaná použití.
...
**AndroidClaw** — Nulová režie. Nulový kompromis. Nasaďte kdekoli. Vyměňte cokoli. 🦀
