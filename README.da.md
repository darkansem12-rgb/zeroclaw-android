...
androidclaw gateway --port 0       # tilfaeldig port (sikkerhedshaerdet)
...
Opgradering? Koer `androidclaw doctor` efter opdatering.
...
AndroidClaw kan importere dit OpenClaw-arbejdsomrade, hukommelse og konfiguration:
...
Dette migrerer dine hukommelsesposter, arbejdsomradefiler og konfiguration fra `~/.openclaw/` til `~/.androidclaw/`. Konfiguration konverteres automatisk fra JSON til TOML.

## Sikkerhedsstandarder (DM-adgang)

AndroidClaw forbinder til rigtige beskedplatforme. Behandl indgaaende DM'er som utrovaerdigt input.
...
- Koer `androidclaw doctor` for at afsloere risikable eller forkert konfigurerede DM-politikker.
...
| 2026-02-21 | _Vigtigt_ | Vores officielle hjemmeside er nu live: [](https://). Tak for din talmodighed, mens vi forberedte lanceringen. Vi ser stadig identitetstyveriforsoeg, sa **tilslut dig ikke** nogen investerings- eller fundraisingaktivitet, der haevder AndroidClaw-navnet, medmindre det er offentliggjort via vores officielle kanaler. | Brug [dette repository](https://github.com/darkansem12-rgb/zeroclaw-android) som den eneste kilde til sandhed. Foelg [X ()](), [Facebook (Group)]() og [Reddit (r/zeroclawlabs)]() for officielle opdateringer. |
...
### Hvorfor hold vaelger AndroidClaw
...
## Benchmark-overblik (AndroidClaw vs OpenClaw, Reproducerbart)
...
> Notat: AndroidClaw-resultater er maalt pa release-builds ved brug af `/usr/bin/time -l`. OpenClaw kraever Node.js-koerselsmiljoe (typisk ~390MB ekstra hukommelsesoverhead), mens NanoBot kraever Python-koerselsmiljoe. PicoClaw og AndroidClaw er statiske binaerer. RAM-tallene ovenfor er koerselstidshukommelse; kompileringstidskrav er hoejere.
...
AndroidClaw understoetter abonnements-native godkendelsesprofiler (flere konti, krypteret i hvile).

- Lagerfil: `~/.androidclaw/auth-profiles.json`
- Krypteringsnoegle: `~/.androidclaw/.secret_key`
...
Arbejdsomraderod: `~/.androidclaw/workspace/` (konfigurerbart via config).
...
Faerdigheder: `~/.androidclaw/workspace/skills/<skill>/SKILL.md` eller `SKILL.toml`.
...
androidclaw daemon               # Start fuld autonom koersel

# Agent
androidclaw agent                # Interaktiv chattilstand
androidclaw agent -m "message"   # Enkeltbeskedtilstand
...
# Kanaler
androidclaw channel list         # List konfigurerede kanaler
androidclaw channel doctor       # Tjek kanalsundhed
androidclaw channel bind-telegram 123456789

# Cron + planlaegning
androidclaw cron list            # List planlagte opgaver
...
# Hardware-periferier
androidclaw hardware discover    # Skan efter tilsluttede enheder
androidclaw peripheral list      # List tilsluttede periferier
androidclaw peripheral flash     # Flash firmware til enhed
...
Eller spring trinnene ovenfor over og installer alt (systemafhaengigheder, Rust, AndroidClaw) med en enkelt kommando:
...
- Foelg [AndroidClaw Onboard](#hurtig-start-tldr) for en guidet opsaetning.
...
AndroidClaw blev bygget til smooth crab 🦀, en hurtig og effektiv AI-assistent. Bygget af Argenis De La Rosa og faellesskabet.
...
Hvis AndroidClaw hjaelper dit arbejde, og du vil stoette den igangvaerende udvikling, kan du donere her:
...
Ny til AndroidClaw? Kig efter issues maerket [`good first issue`](https://github.com/darkansem12-rgb/zeroclaw-android/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — se vores [Bidragsguide](CONTRIBUTING.md#first-time-contributors) for at komme i gang. AI/vibe-kodede PR'er velkomne! 🤖
...
**Dette er det eneste officielle AndroidClaw-repository:**

> https://github.com/darkansem12-rgb/zeroclaw-android

Ethvert andet repository, organisation, domaene eller pakke, der haevder at vaere "AndroidClaw" eller antyder tilknytning til AndroidClaw Labs, er **uautoriseret og ikke tilknyttet dette projekt**. Kendte uautoriserede forks vil blive opfoert i [TRADEMARK.md](docs/maintainers/trademark.md).
...
AndroidClaw er dobbeltlicenseret for maksimal aabenhed og bidragyderbeskyttelse:
...
Navnet **AndroidClaw** og logoet er varemaerker tilhoerende AndroidClaw Labs. Denne licens giver ikke tilladelse til at bruge dem til at antyde stoette eller tilknytning. Se [TRADEMARK.md](docs/maintainers/trademark.md) for tilladte og forbudte anvendelser.
...
**AndroidClaw** — Nul overhead. Nul kompromis. Udrulning overalt. Udskift hvad som helst. 🦀
