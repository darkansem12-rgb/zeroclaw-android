...
<h1 align="center">🦀 AndroidClaw — パーソナルAIアシスタント</h1>
...
> **推奨セットアップ：** ターミナルで `androidclaw onboard` を実行してください。AndroidClaw Onboardがゲートウェイ、ワークスペース、チャンネル、プロバイダーのセットアップをステップバイステップでガイドします。これは推奨されるセットアップパスで、macOS、Linux、Windows（WSL2経由）で動作します。新規インストール？ここから開始：[はじめに](#クイックスタートtldr)
...
`androidclaw onboard` はインストール後に自動的に実行され、ワークスペースとプロバイダーを設定します。
...
androidclaw gateway                # デフォルト：127.0.0.1:42617
androidclaw gateway --port 0       # ランダムポート（セキュリティ強化）
...
アップグレード？更新後に `androidclaw doctor` を実行してください。
...
AndroidClawはOpenClawのワークスペース、メモリ、設定をインポートできます：
...
これにより、メモリエントリ、ワークスペースファイル、設定が `~/.openclaw/` から `~/.androidclaw/` に移行されます。設定はJSONからTOMLに自動変換されます。

## セキュリティデフォルト（DMアクセス）

AndroidClawは実際のメッセージングサービスに接続します。着信DMを信頼できない入力として扱ってください。
...
- `androidclaw doctor` を実行してリスクのある、または設定ミスのあるDMポリシーを検出します。
...
| 日付 (UTC) | レベル | 通知 | 対応 |
| ---------- | ------ | ---- | ---- |
| 2026-02-19 | _重大_ | 当プロジェクトは `openagen/zeroclaw`、`zeroclaw.org`、`zeroclaw.net` とは**一切関係ありません**。`zeroclaw.org` と `zeroclaw.net` ドメインは現在 `openagen/zeroclaw` フォークを指しており、そのドメイン/リポジトリは当プロジェクトの公式ウェブサイト/プロジェクトを偽装しています。 | それらのソースからの情報、バイナリ、資金調達、告知を信頼しないでください。[このリポジトリ](https://github.com/darkansem12-rgb/zeroclaw-android)と認証済みのソーシャルアカウントのみを使用してください。 |
| 2026-02-21 | _重要_ | 公式ウェブサイトが公開されました：[](https://)。ローンチ準備中のお待ちいただき、ありがとうございます。引き続き偽装行為が確認されていますので、公式チャンネルを通じて公開されない限り、AndroidClawの名前を使った投資や資金調達活動には**参加しないでください**。 | [このリポジトリ](https://github.com/darkansem12-rgb/zeroclaw-android)を唯一の信頼できる情報源として使用してください。公式アップデートは [X ()]()、[Facebook (Group)]()、[Reddit (r/zeroclawlabs)]() をフォローしてください。 |
...
## ベンチマークスナップショット（AndroidClaw vs OpenClaw、再現可能）
...
> 注意：AndroidClawの結果はリリースビルドで `/usr/bin/time -l` を使用して測定されています。OpenClawにはNode.jsランタイム（通常約390MBの追加メモリオーバーヘッド）が必要で、NanoBotにはPythonランタイムが必要です。PicoClawとAndroidClawは静的バイナリです。上記のRAM数値はランタイムメモリです。ビルド時のコンパイル要件はより高くなります。
...
AndroidClawはサブスクリプションネイティブ認証プロファイル（マルチアカウント、保存時暗号化）をサポートしています。

- ストアファイル：`~/.androidclaw/auth-profiles.json`
- 暗号化キー：`~/.androidclaw/.secret_key`
...
ワークスペースルート：`~/.androidclaw/workspace/`（設定で変更可能）。
...
スキル：`~/.androidclaw/workspace/skills/<skill>/SKILL.md` または `SKILL.toml`。
...
```bash
# ワークスペース管理
androidclaw onboard              # ガイド付きセットアップウィザード
androidclaw status               # デーモン/エージェントのステータス表示
androidclaw doctor               # システム診断を実行

# ゲートウェイ + デーモン
androidclaw gateway              # ゲートウェイサーバーを起動（127.0.0.1:42617）
androidclaw daemon               # フル自律ランタイムを起動

# エージェント
androidclaw agent                # インタラクティブチャットモード
androidclaw agent -m "message"   # 単一メッセージモード
...
# チャンネル
androidclaw channel list         # 設定済みチャンネルの一覧
androidclaw channel doctor       # チャンネルヘルスの確認
androidclaw channel bind-telegram 123456789

# Cron + スケジューリング
androidclaw cron list            # スケジュールタスクの一覧
...
# ハードウェア周辺機器
androidclaw hardware discover    # 接続デバイスのスキャン
androidclaw peripheral list      # 接続周辺機器の一覧
androidclaw peripheral flash     # デバイスへのファームウェア書き込み
...
または、上記のステップをスキップして、単一コマンドですべてをインストール（システム依存、Rust、AndroidClaw）：
...
- [AndroidClaw Onboard](#クイックスタートtldr)でガイド付きセットアップ。
...
AndroidClawはsmooth crab 🦀のために構築されました。高速で効率的なAIアシスタント。Argenis De La Rosaとコミュニティによって構築されました。
...
AndroidClawがあなたの仕事に役立ち、継続的な開発を支援したい場合は、こちらから寄付できます：
...
AndroidClaw初心者ですか？[`good first issue`](https://github.com/darkansem12-rgb/zeroclaw-android/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) ラベルの付いた課題を探してください — 始め方は[貢献ガイド](CONTRIBUTING.md#first-time-contributors)を参照。AI/vibe-coded PRも歓迎します！🤖
...
- 新 `Skill` → `~/.androidclaw/workspace/skills/<name>/`
...
**これがAndroidClawの唯一の公式リポジトリです：**

> https://github.com/darkansem12-rgb/zeroclaw-android

「AndroidClaw」を名乗る、またはAndroidClaw Labsとの提携を示唆する他のリポジトリ、組織、ドメイン、パッケージは**無許可であり、本プロジェクトとは無関係です**。既知の無許可フォークは [TRADEMARK.md](docs/maintainers/trademark.md) に記載されます。

偽装や商標の悪用を見つけた場合は、[issueを作成](https://github.com/darkansem12-rgb/zeroclaw-android/issues)してください。

---

## ライセンス

AndroidClawは最大限のオープン性と貢献者保護のためにデュアルライセンスです：
...
**AndroidClaw** の名称とロゴはAndroidClaw Labsの商標です。このライセンスは、推薦や提携を暗示するための使用許可を付与しません。許可された使用と禁止された使用については [TRADEMARK.md](docs/maintainers/trademark.md) を参照してください。
...
**AndroidClaw** — ゼロオーバーヘッド。ゼロ妥協。どこでもデプロイ。何でも交換。🦀
