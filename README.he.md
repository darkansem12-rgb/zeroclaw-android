...
<h1 align="center">🦀 AndroidClaw — עוזר AI אישי</h1>
...
משדרג? הרץ `androidclaw doctor` לאחר העדכון.
...
AndroidClaw יכול לייבא את סביבת העבודה, הזיכרון וההגדרות של OpenClaw שלך:
...
זה מעביר את רשומות הזיכרון, קבצי סביבת העבודה וההגדרות מ-`~/.openclaw/` ל-`~/.androidclaw/`. ההגדרות מומרות אוטומטית מ-JSON ל-TOML.

## ברירות מחדל אבטחה (גישת DM)

AndroidClaw מתחבר למשטחי הודעות אמיתיים. התייחס ל-DM נכנסים כקלט לא מהימן.
...
- הרץ `androidclaw doctor` כדי לחשוף מדיניות DM מסוכנת או שגויה.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _קריטי_ | אנחנו **לא מזוהים** עם `openagen/zeroclaw`, `zeroclaw.org` או `zeroclaw.net`. הדומיינים `zeroclaw.org` ו-`zeroclaw.net` מפנים כרגע ל-fork `openagen/zeroclaw`, ואותו דומיין/מאגר מתחזים לאתר/פרויקט הרשמי שלנו. | אל תסמוך על מידע, בינאריים, גיוס כספים או הודעות ממקורות אלה. השתמש רק ב[מאגר זה](https://github.com/darkansem12-rgb/zeroclaw-android) ובחשבונות החברתיים המאומתים שלנו. |
| 2026-02-21 | _חשוב_ | האתר הרשמי שלנו כעת פעיל: [](https://). תודה על הסבלנות בזמן שהכנו את ההשקה. אנחנו עדיין רואים ניסיונות התחזות, לכן **אל** תצטרפו לפעילות השקעה או גיוס כספים הטוענת לשם AndroidClaw אלא אם היא מפורסמת דרך הערוצים הרשמיים שלנו. | השתמש ב[מאגר זה](https://github.com/darkansem12-rgb/zeroclaw-android) כמקור האמת היחיד. עקוב אחרי [X ()](), [Facebook (Group)]() ו-[Reddit (r/zeroclawlabs)]() לעדכונים רשמיים. |
...
### למה צוותים בוחרים ב-AndroidClaw
...
## תמונת מצב של ביצועים (AndroidClaw מול OpenClaw, ניתן לשחזור)
...
> הערות: תוצאות AndroidClaw נמדדו על בנייות שחרור באמצעות `/usr/bin/time -l`. OpenClaw דורש סביבת ריצה Node.js (בדרך כלל ~390MB תקורת זיכרון נוספת), בעוד NanoBot דורש סביבת ריצה Python. PicoClaw ו-AndroidClaw הם בינאריים סטטיים. נתוני ה-RAM למעלה הם זיכרון סביבת ריצה; דרישות קומפילציה בזמן בנייה גבוהות יותר.
...
מינימלי `~/.androidclaw/config.toml`:
...
AndroidClaw תומך בפרופילי אימות מקוריים למנוי (רב-חשבוני, מוצפן במנוחה).

- קובץ אחסון: `~/.androidclaw/auth-profiles.json`
- מפתח הצפנה: `~/.androidclaw/.secret_key`
...
שורש סביבת עבודה: `~/.androidclaw/workspace/` (ניתן להגדרה דרך ההגדרות).
...
מיומנויות: `~/.androidclaw/workspace/skills/<skill>/SKILL.md` או `SKILL.toml`.
...
או דלג על השלבים למעלה והתקן הכל (תלויות מערכת, Rust, AndroidClaw) בפקודה אחת:
...
- עקוב אחרי [AndroidClaw Onboard](#התחלה-מהירה) להגדרה מונחית.
...
AndroidClaw נבנה עבור ה-smooth crab 🦀, עוזר AI מהיר ויעיל. נבנה על ידי Argenis De La Rosa והקהילה.
...
אם AndroidClaw עוזר לעבודה שלך ואתה רוצה לתמוך בפיתוח המתמשך, אתה יכול לתרום כאן:
...
חדש ב-AndroidClaw? חפש בעיות עם התווית [`good first issue`](https://github.com/darkansem12-rgb/zeroclaw-android/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — ראה את [מדריך התרומה](CONTRIBUTING.md#first-time-contributors) שלנו כדי להתחיל. PR של AI/vibe-coded מתקבלים בברכה! 🤖
...
- `Skill` חדש → `~/.androidclaw/workspace/skills/<name>/`
...
**זהו מאגר AndroidClaw הרשמי היחיד:**

> https://github.com/darkansem12-rgb/zeroclaw-android

כל מאגר, ארגון, דומיין או חבילה אחרים הטוענים להיות "AndroidClaw" או מרמזים על שיוך ל-AndroidClaw Labs הם **לא מורשים ולא מזוהים עם פרויקט זה**. פורקים לא מורשים ידועים ירשמו ב-[TRADEMARK.md](docs/maintainers/trademark.md).

אם אתה נתקל בהתחזות או שימוש לרעה בסימן מסחרי, אנא [פתח issue](https://github.com/darkansem12-rgb/zeroclaw-android/issues).

---

## רישיון

AndroidClaw מורשה ברישיון כפול לפתיחות מקסימלית והגנה על תורמים:
...
השם והלוגו של **AndroidClaw** הם סימנים מסחריים של AndroidClaw Labs. רישיון זה אינו מעניק הרשאה להשתמש בהם כדי לרמוז על תמיכה או שיוך. ראה [TRADEMARK.md](docs/maintainers/trademark.md) לשימושים מותרים ואסורים.
...
**AndroidClaw** — אפס תקורה. אפס פשרות. פרוס בכל מקום. החלף הכל. 🦀
