...
androidclaw gateway --port 0       # tilfeldig port (sikkerhetsskarmet)
...
Oppgraderer? Kjor `androidclaw doctor` etter oppdatering.
...
AndroidClaw kan importere ditt OpenClaw-arbeidsomrade, minne og konfigurasjon:
...
Dette migrerer minneoppforinger, arbeidsomradefiler og konfigurasjon fra `~/.openclaw/` til `~/.androidclaw/`. Konfigurasjon konverteres automatisk fra JSON til TOML.

## Sikkerhetsstandarder (DM-tilgang)

AndroidClaw kobler til ekte meldingsflater. Behandle innkommende DM-er som upalitelig inndata.
...
- Kjor `androidclaw doctor` for a avdekke risikable eller feilkonfigurerte DM-policyer.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritisk_ | Vi er **ikke tilknyttet** `openagen/zeroclaw`, `zeroclaw.org` eller `zeroclaw.net`. Domenene `zeroclaw.org` og `zeroclaw.net` peker for oyeblikket til `openagen/zeroclaw`-forken, og dette domenet/repositoriet utgir seg for a vaere vart offisielle nettsted/prosjekt. | Ikke stol pa informasjon, binarfiler, innsamlinger eller kunngoringer fra disse kildene. Bruk kun [dette repositoriet](https://github.com/darkansem12-rgb/zeroclaw-android) og vare verifiserte sosiale kontoer. |
| 2026-02-21 | _Viktig_ | Vart offisielle nettsted er na live: [](https://). Takk for talmodigheten mens vi forberedte lanseringen. Vi ser fortsatt etterligningsforsok, sa **ikke** bli med pa noen investerings- eller innsamlingsaktivitet som hevder AndroidClaw-navnet med mindre det er publisert gjennom vare offisielle kanaler. | Bruk [dette repositoriet](https://github.com/darkansem12-rgb/zeroclaw-android) som eneste sannhetskilde. Folg [X ()](), [Facebook (Gruppe)]() og [Reddit (r/zeroclawlabs)]() for offisielle oppdateringer. |
...
### Hvorfor team velger AndroidClaw
...
## Ytelsessammenligning (AndroidClaw vs OpenClaw, reproduserbar)
...
> Merknader: AndroidClaw-resultater er malt pa release-bygg med `/usr/bin/time -l`. OpenClaw krever Node.js-kjoretidemiljo (typisk ~390MB ekstra minneoverhead), mens NanoBot krever Python-kjoretidemiljo. PicoClaw og AndroidClaw er statiske binarfiler. RAM-tallene ovenfor er kjoretidesminne; byggetidskompileringskrav er hoyere.

<p align="center">
  <img src="docs/assets/zeroclaw-comparison.jpeg" alt="AndroidClaw vs OpenClaw-sammenligning" width="800" />
</p>
...
AndroidClaw stotter abonnements-native autentiseringsprofiler (multi-konto, kryptert i hvile).

- Lagringsfil: `~/.androidclaw/auth-profiles.json`
- Krypteringsnokkel: `~/.androidclaw/.secret_key`
...
Arbeidsomraderot: `~/.androidclaw/workspace/` (konfigurerbar via konfigurasjon).
...
Ferdigheter: `~/.androidclaw/workspace/skills/<skill>/SKILL.md` eller `SKILL.toml`.
...
androidclaw daemon               # Start full autonom kjoretidemiljo

# Agent
androidclaw agent                # Interaktiv chatmodus
androidclaw agent -m "melding"   # Enkeltmeldingsmodus
...
# Kanaler
androidclaw channel list         # List konfigurerte kanaler
androidclaw channel doctor       # Sjekk kanalhelse
androidclaw channel bind-telegram 123456789

# Cron + planlegging
androidclaw cron list            # List planlagte jobber
androidclaw cron add "*/5 * * * *" --prompt "Sjekk systemhelse"
...
androidclaw auth login --provider <navn>
androidclaw auth status
androidclaw auth use --provider <navn> --profile <profil>

# Maskinvareperiferiutstyr
androidclaw hardware discover    # Sok etter tilkoblede enheter
androidclaw peripheral list      # List tilkoblede periferienheter
androidclaw peripheral flash     # Flash firmware til enhet
...
Eller hopp over stegene ovenfor og installer alt (systemavhengigheter, Rust, AndroidClaw) med en enkelt kommando:
...
- Folg [AndroidClaw Onboard](#hurtigstart) for et veiledet oppsett.
...
AndroidClaw ble bygget for den smidige krabben 🦀, en rask og effektiv AI-assistent. Bygget av Argenis De La Rosa og fellesskapet.
...
Hvis AndroidClaw hjelper arbeidet ditt og du onsker a stotte pagaende utvikling, kan du donere her:
...
Ny til AndroidClaw? Se etter issues merket [`good first issue`](https://github.com/darkansem12-rgb/zeroclaw-android/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — se var [Bidragsguide](CONTRIBUTING.md#first-time-contributors) for hvordan du kommer i gang. AI/vibe-kodede PR-er er velkomne! 🤖
...
**Dette er det eneste offisielle AndroidClaw-repositoriet:**

> https://github.com/darkansem12-rgb/zeroclaw-android

Ethvert annet repository, organisasjon, domene eller pakke som hevder a vaere "AndroidClaw" eller antyder tilknytning til AndroidClaw Labs er **uautorisert og ikke tilknyttet dette prosjektet**. Kjente uautoriserte forker vil bli listet i [TRADEMARK.md](docs/maintainers/trademark.md).

Hvis du stoter pa etterligning eller varemerkemisbruk, vennligst [opprett en issue](https://github.com/darkansem12-rgb/zeroclaw-android/issues).

---

## Lisens

AndroidClaw er dobbelt-lisensiert for maksimal apenhet og bidragsyterbeskyttelse:
...
**AndroidClaw**-navnet og logoen er varemerker for AndroidClaw Labs. Denne lisensen gir ikke tillatelse til a bruke dem for a antyde stotte eller tilknytning. Se [TRADEMARK.md](docs/maintainers/trademark.md) for tillatt og forbudt bruk.
...
**AndroidClaw** — Null overhead. Null kompromiss. Distribuer overalt. Bytt hva som helst. 🦀

## Bidragsytere

<a href="https://github.com/darkansem12-rgb/zeroclaw-android/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=darkansem12-rgb/zeroclaw-android" alt="AndroidClaw-bidragsytere" />
</a>
...
<p align="center">
  <a href="https://www.star-history.com/#zeroclaw-labs/zeroclaw&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Stjernehistorikk-diagram" src="https://api.star-history.com/svg?repos=darkansem12-rgb/zeroclaw-android&type=date&legend=top-left" />
    </picture>
  </a>
</p>
