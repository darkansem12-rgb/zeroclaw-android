...
<h1 align="center">🦀 AndroidClaw — ผู้ช่วย AI ส่วนตัว</h1>
...
> **การตั้งค่าที่แนะนำ:** รัน `androidclaw onboard` ในเทอร์มินัลของคุณ AndroidClaw Onboard จะแนะนำคุณทีละขั้นตอนในการตั้งค่า gateway, workspace, ช่องทาง และ provider เป็นเส้นทางการตั้งค่าที่แนะนำและใช้งานได้บน macOS, Linux และ Windows (ผ่าน WSL2) ติดตั้งใหม่? เริ่มที่นี่: [เริ่มต้นใช้งาน](#เริ่มต้นอย่างรวดเร็ว)
...
`androidclaw onboard` จะรันโดยอัตโนมัติหลังติดตั้งเพื่อกำหนดค่า workspace และ provider ของคุณ
...
androidclaw gateway                # ค่าเริ่มต้น: 127.0.0.1:42617
androidclaw gateway --port 0       # พอร์ตสุ่ม (ความปลอดภัยเพิ่มขึ้น)
...
กำลังอัปเกรด? รัน `androidclaw doctor` หลังจากอัปเดต
...
AndroidClaw สามารถนำเข้า workspace, หน่วยความจำ และการกำหนดค่าจาก OpenClaw ของคุณ:
...
สิ่งนี้จะย้ายรายการหน่วยความจำ ไฟล์ workspace และการกำหนดค่าจาก `~/.openclaw/` ไปยัง `~/.androidclaw/` การกำหนดค่าจะถูกแปลงจาก JSON เป็น TOML โดยอัตโนมัติ

## ค่าเริ่มต้นด้านความปลอดภัย (การเข้าถึง DM)

AndroidClaw เชื่อมต่อกับพื้นผิวการส่งข้อความจริง ถือว่า DM ขาเข้าเป็นข้อมูลที่ไม่น่าเชื่อถือ
...
- รัน `androidclaw doctor` เพื่อค้นหานโยบาย DM ที่เสี่ยงหรือกำหนดค่าผิด
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _วิกฤต_  | เรา**ไม่มีส่วนเกี่ยวข้อง**กับ `openagen/zeroclaw`, `zeroclaw.org` หรือ `zeroclaw.net` โดเมน `zeroclaw.org` และ `zeroclaw.net` ปัจจุบันชี้ไปที่ fork `openagen/zeroclaw` และโดเมน/repository เหล่านั้นกำลังปลอมตัวเป็นเว็บไซต์/โปรเจกต์อย่างเป็นทางการของเรา                                                                                       | อย่าเชื่อถือข้อมูล ไบนารี การระดมทุน หรือประกาศจากแหล่งเหล่านั้น ใช้เฉพาะ[repository นี้](https://github.com/darkansem12-rgb/zeroclaw-android)และบัญชีโซเชียลที่ได้รับการยืนยันของเรา                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 2026-02-21 | _สำคัญ_ | เว็บไซต์อย่างเป็นทางการของเราพร้อมใช้งานแล้ว: [](https://) ขอบคุณสำหรับความอดทนขณะที่เราเตรียมการเปิดตัว เรายังคงเห็นความพยายามในการแอบอ้าง ดังนั้น**อย่า**เข้าร่วมกิจกรรมการลงทุนหรือระดมทุนที่อ้างชื่อ AndroidClaw เว้นแต่จะเผยแพร่ผ่านช่องทางอย่างเป็นทางการของเรา                            | ใช้[repository นี้](https://github.com/darkansem12-rgb/zeroclaw-android)เป็นแหล่งข้อมูลที่เชื่อถือได้เพียงแหล่งเดียว ติดตาม [X ()](), [Facebook (Group)]() และ [Reddit (r/zeroclawlabs)]() สำหรับอัปเดตอย่างเป็นทางการ |
...
## สรุป Benchmark (AndroidClaw vs OpenClaw, ทำซ้ำได้)
...
> หมายเหตุ: ผลลัพธ์ AndroidClaw วัดจาก release builds โดยใช้ `/usr/bin/time -l` OpenClaw ต้องการ runtime Node.js (โดยทั่วไป ~390MB overhead หน่วยความจำเพิ่มเติม) ในขณะที่ NanoBot ต้องการ runtime Python PicoClaw และ AndroidClaw เป็นไบนารีแบบ static ตัวเลข RAM ด้านบนเป็นหน่วยความจำ runtime ความต้องการการคอมไพล์ตอน build สูงกว่า
...
ขั้นต่ำ `~/.androidclaw/config.toml`:
...
AndroidClaw รองรับโปรไฟล์การยืนยันตัวตนแบบ subscription-native (หลายบัญชี, เข้ารหัสเมื่อเก็บ)

- ไฟล์จัดเก็บ: `~/.androidclaw/auth-profiles.json`
- คีย์เข้ารหัส: `~/.androidclaw/.secret_key`
...
Root workspace: `~/.androidclaw/workspace/` (กำหนดค่าได้ผ่าน config)
...
Skills: `~/.androidclaw/workspace/skills/<skill>/SKILL.md` หรือ `SKILL.toml`
...
```bash
# การจัดการ workspace
androidclaw onboard              # วิซาร์ดการตั้งค่าแบบแนะนำ
androidclaw status               # แสดงสถานะ daemon/เอเจนต์
androidclaw doctor               # รันการวินิจฉัยระบบ

# Gateway + daemon
androidclaw gateway              # เริ่มเซิร์ฟเวอร์ gateway (127.0.0.1:42617)
androidclaw daemon               # เริ่ม runtime อัตโนมัติเต็มรูปแบบ

# เอเจนต์
androidclaw agent                # โหมดแชทโต้ตอบ
androidclaw agent -m "message"   # โหมดข้อความเดียว
...
# ช่องทาง
androidclaw channel list         # แสดงรายการช่องทางที่กำหนดค่า
androidclaw channel doctor       # ตรวจสอบสุขภาพช่องทาง
androidclaw channel bind-telegram 123456789

# Cron + การกำหนดเวลา
androidclaw cron list            # แสดงรายการงานที่กำหนดเวลา
...
# อุปกรณ์ต่อพ่วง
androidclaw hardware discover    # สแกนอุปกรณ์ที่เชื่อมต่อ
androidclaw peripheral list      # แสดงรายการอุปกรณ์ต่อพ่วงที่เชื่อมต่อ
androidclaw peripheral flash     # แฟลช firmware ไปยังอุปกรณ์
...
หรือข้ามขั้นตอนด้านบนและติดตั้งทุกอย่าง (dependencies ระบบ, Rust, AndroidClaw) ในคำสั่งเดียว:
...
- ทำตาม [AndroidClaw Onboard](#เริ่มต้นอย่างรวดเร็ว) สำหรับการตั้งค่าแบบแนะนำ
...
AndroidClaw ถูกสร้างสำหรับ smooth crab 🦀 ผู้ช่วย AI ที่เร็วและมีประสิทธิภาพ สร้างโดย Argenis De La Rosa และชุมชน

- [](https://)
- [](https://x.com/zeroclawlabs)

## สนับสนุน AndroidClaw

หาก AndroidClaw ช่วยงานของคุณและคุณต้องการสนับสนุนการพัฒนาต่อเนื่อง คุณสามารถบริจาคที่นี่:
...
ใหม่กับ AndroidClaw? มองหา issues ที่มีป้ายกำกับ [`good first issue`](https://github.com/darkansem12-rgb/zeroclaw-android/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — ดู[คู่มือการมีส่วนร่วม](CONTRIBUTING.md#first-time-contributors)สำหรับวิธีเริ่มต้น ยินดีรับ PR ที่สร้างด้วย AI/vibe-coded! 🤖
...
- `Skill` ใหม่ → `~/.androidclaw/workspace/skills/<name>/`
...
**นี่คือ repository อย่างเป็นทางการเพียงแห่งเดียวของ AndroidClaw:**

> https://github.com/darkansem12-rgb/zeroclaw-android

repository, องค์กร, โดเมน หรือแพ็กเกจอื่นใดที่อ้างว่าเป็น "AndroidClaw" หรือบ่งบอกถึงการเกี่ยวข้องกับ AndroidClaw Labs นั้น**ไม่ได้รับอนุญาตและไม่มีส่วนเกี่ยวข้องกับโปรเจกต์นี้** Fork ที่ไม่ได้รับอนุญาตที่ทราบจะถูกระบุไว้ใน [TRADEMARK.md](docs/maintainers/trademark.md)

หากคุณพบการแอบอ้างหรือการใช้เครื่องหมายการค้าในทางที่ผิด โปรด[เปิด issue](https://github.com/darkansem12-rgb/zeroclaw-android/issues)

---

## สัญญาอนุญาต

AndroidClaw มี dual-license เพื่อความเปิดกว้างสูงสุดและการปกป้องผู้มีส่วนร่วม:
...
ชื่อและโลโก้ **AndroidClaw** เป็นเครื่องหมายการค้าของ AndroidClaw Labs สัญญาอนุญาตนี้ไม่ให้สิทธิ์ในการใช้เพื่อบ่งบอกถึงการรับรองหรือการเกี่ยวข้อง ดู [TRADEMARK.md](docs/maintainers/trademark.md) สำหรับการใช้งานที่อนุญาตและห้าม
...
**AndroidClaw** — ไม่มีโอเวอร์เฮด ไม่มีการประนีประนอม Deploy ที่ไหนก็ได้ สลับอะไรก็ได้ 🦀
