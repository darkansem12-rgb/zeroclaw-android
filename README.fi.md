...
<h1 align="center">🦀 AndroidClaw — Henkilokohtainen tekoalyavustaja</h1>
...
androidclaw gateway                # oletus: 127.0.0.1:42617
androidclaw gateway --port 0       # satunnainen portti (turvallisuuskovennettu)
...
Paivitat? Suorita `androidclaw doctor` paivityksen jalkeen.
...
AndroidClaw voi tuoda OpenClaw-tyotilasi, muistisi ja maaritykset:
...
Tama siirtaa muistimerkinnot, tyotilan tiedostot ja maaritykset hakemistosta `~/.openclaw/` hakemistoon `~/.androidclaw/`. Maaritykset muunnetaan automaattisesti JSON:sta TOML:ksi.

## Turvallisuuden oletusasetukset (DM-paasy)

AndroidClaw yhdistaa todellisiin viestintapintoihin. Kasittele saapuvia DM-viesteja luottamattomana syotteena.
...
- Suorita `androidclaw doctor` tunnistaaksesi riskilliset tai vaarinkonfiguroidut DM-kaytannot.
...
| 2026-02-21 | _Tarkea_ | Virallinen verkkosivustomme on nyt toiminnassa: [](https://). Kiitos karsivallisyydestanne julkaisun valmistelun aikana. Nakemme edelleen esiintymisyrityksia, joten **ala** liity mihinkaan sijoitus- tai varainkeruutoimintaan, joka vaittaa AndroidClaw-nimea, ellei se ole julkaistu virallisten kanaviemme kautta. | Kayta [tata varastoa](https://github.com/darkansem12-rgb/zeroclaw-android) ainoana totuuden lahteena. Seuraa [X ()](), [Facebook (Group)]() ja [Reddit (r/zeroclawlabs)]() virallisia paivityksia varten. |
...
### Miksi tiimit valitsevat AndroidClaw:n
...
## Vertailun tilannekuva (AndroidClaw vs OpenClaw, Toistettava)
...
> Huomautukset: AndroidClaw-tulokset mitattu release-buildeilla kayttaen `/usr/bin/time -l`. OpenClaw vaatii Node.js-ajoympariston (tyypillisesti ~390MB ylimaaraista muistikuormaa), kun taas NanoBot vaatii Python-ajoympariston. PicoClaw ja AndroidClaw ovat staattisia binaareja. Yllaolevat RAM-luvut ovat ajoaikaista muistia; kaannosaikaiset vaatimukset ovat korkeammat.
...
Minimaalinen `~/.androidclaw/config.toml`:
...
AndroidClaw tukee tilausnatiiveja tunnistautumisprofiileja (useita tileja, salattu levossa).

- Tallennustiedosto: `~/.androidclaw/auth-profiles.json`
- Salausavain: `~/.androidclaw/.secret_key`
...
Tyotilan juuri: `~/.androidclaw/workspace/` (konfiguroitavissa maaritysten kautta).
...
Taidot: `~/.androidclaw/workspace/skills/<skill>/SKILL.md` tai `SKILL.toml`.
...
# Gateway + daemon
androidclaw gateway              # Kaynnista gateway-palvelin (127.0.0.1:42617)
androidclaw daemon               # Kaynnista taysi autonominen ajoymparisto

# Agentti
androidclaw agent                # Interaktiivinen keskustelutila
androidclaw agent -m "message"   # Yksittaisen viestin tila
...
# Kanavat
androidclaw channel list         # Listaa konfiguroidut kanavat
androidclaw channel doctor       # Tarkista kanavien terveys
androidclaw channel bind-telegram 123456789

# Cron + ajastus
androidclaw cron list            # Listaa ajastetut tehtavat
...
androidclaw hardware discover    # Etsi yhdistettuja laitteita
androidclaw peripheral list      # Listaa yhdistetyt periferiat
androidclaw peripheral flash     # Flash-ohjelma laitteeseen
...
Tai ohita yllaolevat vaiheet ja asenna kaikki (jarjestelmariippuvuudet, Rust, AndroidClaw) yhdella komennolla:
...
- Noudata [AndroidClaw Onboard](#pikaaloitus-tldr) -palvelua opastettuun asennukseen.
...
AndroidClaw rakennettiin smooth crab 🦀 -kaveria varten, nopea ja tehokas tekoalyavustaja. Rakennettu Argenis De La Rosan ja yhteison toimesta.
...
Jos AndroidClaw auttaa tyossasi ja haluat tukea jatkuvaa kehitysta, voit lahjoittaa tassa:
...
Uusi AndroidClaw:ssa? Etsi issueita merkinnalla [`good first issue`](https://github.com/darkansem12-rgb/zeroclaw-android/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — katso [Osallistumisoppaamme](CONTRIBUTING.md#first-time-contributors) aloittaaksesi. AI/vibe-koodatut PR:t tervetulleita! 🤖
...
- Uusi `Skill` → `~/.androidclaw/workspace/skills/<name>/`
...
**Tama on ainoa virallinen AndroidClaw-varasto:**

> https://github.com/darkansem12-rgb/zeroclaw-android

Mika tahansa muu varasto, organisaatio, verkkotunnus tai paketti, joka vaittaa olevansa "AndroidClaw" tai viittaa yhteyteen AndroidClaw Labsin kanssa, on **luvaton eika liity tahan projektiin**. Tunnetut luvattomat forkit listataan [TRADEMARK.md](docs/maintainers/trademark.md)-tiedostossa.
...
AndroidClaw on kaksoislisenssoitu maksimaalisen avoimuuden ja osallistujien suojan takaamiseksi:
...
**AndroidClaw**-nimi ja logo ovat AndroidClaw Labsin tavaramerkkeja. Tama lisenssi ei anna lupaa kayttaa niita tuen tai yhteyden vihjamiseen. Katso [TRADEMARK.md](docs/maintainers/trademark.md) sallittujen ja kiellettyjen kayttojen osalta.
...
**AndroidClaw** — Nolla ylimaaraa. Nolla kompromisseja. Kayttoönotto minne tahansa. Vaihda mita tahansa. 🦀
