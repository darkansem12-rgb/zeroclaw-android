...
<h1 align="center">🦀 AndroidClaw — Assistente Pessoal de IA</h1>
...
androidclaw gateway                # padrão: 127.0.0.1:42617
androidclaw gateway --port 0       # porta aleatória (segurança reforçada)
...
Atualizando? Execute `androidclaw doctor` após atualizar.
...
O AndroidClaw pode importar seu workspace, memória e configuração do OpenClaw:
...
Isso migra suas entradas de memória, arquivos do workspace e configuração de `~/.openclaw/` para `~/.androidclaw/`. A configuração é convertida de JSON para TOML automaticamente.

## Padrões de segurança (acesso por DM)

O AndroidClaw conecta-se a superfícies de mensagens reais. Trate DMs recebidas como entrada não confiável.
...
- Execute `androidclaw doctor` para detectar políticas de DM arriscadas ou mal configuradas.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Crítico_  | **Não somos afiliados** a `openagen/zeroclaw`, `zeroclaw.org` ou `zeroclaw.net`. Os domínios `zeroclaw.org` e `zeroclaw.net` atualmente apontam para o fork `openagen/zeroclaw`, e esse domínio/repositório estão se passando pelo nosso site/projeto oficial.                                                                                       | Não confie em informações, binários, arrecadações de fundos ou anúncios dessas fontes. Use apenas [este repositório](https://github.com/darkansem12-rgb/zeroclaw-android) e nossas contas sociais verificadas.                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 2026-02-21 | _Importante_ | Nosso site oficial agora está no ar: [](https://). Obrigado pela paciência enquanto preparávamos o lançamento. Continuamos vendo tentativas de falsificação, então **não** participe de atividades de investimento ou arrecadação de fundos usando o nome AndroidClaw, a menos que sejam publicadas através dos nossos canais oficiais.                            | Use [este repositório](https://github.com/darkansem12-rgb/zeroclaw-android) como a única fonte de verdade. Siga [X ()](), [Facebook (Grupo)]() e [Reddit (r/zeroclawlabs)]() para atualizações oficiais. |
...
### Por que equipes escolhem o AndroidClaw
...
## Resumo de benchmarks (AndroidClaw vs OpenClaw, reproduzível)
...
> Notas: Os resultados do AndroidClaw são medidos em builds release usando `/usr/bin/time -l`. O OpenClaw requer o runtime Node.js (tipicamente ~390MB de overhead adicional de memória), enquanto o NanoBot requer o runtime Python. PicoClaw e AndroidClaw são binários estáticos. Os valores de RAM acima são memória em runtime; os requisitos de compilação são maiores.
...
O AndroidClaw suporta perfis de autenticação nativos de assinatura (multi-conta, criptografados em repouso).

- Arquivo de armazenamento: `~/.androidclaw/auth-profiles.json`
- Chave de criptografia: `~/.androidclaw/.secret_key`
...
Raiz do workspace: `~/.androidclaw/workspace/` (configurável via config).
...
androidclaw daemon               # Iniciar runtime autônomo completo

# Agente
androidclaw agent                # Modo de chat interativo
androidclaw agent -m "message"   # Modo de mensagem única
...
# Canais
androidclaw channel list         # Listar canais configurados
androidclaw channel doctor       # Verificar saúde dos canais
androidclaw channel bind-telegram 123456789

# Cron + agendamento
androidclaw cron list            # Listar trabalhos agendados
...
androidclaw peripheral flash     # Flashear firmware no dispositivo
...
Ou pule os passos acima e instale tudo (dependências do sistema, Rust, AndroidClaw) em um único comando:
...
- Siga o [AndroidClaw Onboard](#início-rápido) para uma configuração guiada.
...
O AndroidClaw foi construído para o caranguejo suave 🦀, um assistente de IA rápido e eficiente. Construído por Argenis De La Rosa e a comunidade.
...
Se o AndroidClaw ajuda no seu trabalho e você quer apoiar o desenvolvimento contínuo, pode doar aqui:
...
Novo no AndroidClaw? Procure issues rotulados como [`good first issue`](https://github.com/darkansem12-rgb/zeroclaw-android/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — veja nosso [Guia de contribuição](CONTRIBUTING.md#first-time-contributors) para saber como começar. PRs com IA/vibe-coded são bem-vindos! 🤖
...
- Novo `Skill` → `~/.androidclaw/workspace/skills/<name>/`
...
**Este é o único repositório oficial do AndroidClaw:**

> https://github.com/darkansem12-rgb/zeroclaw-android

Qualquer outro repositório, organização, domínio ou pacote que afirme ser "AndroidClaw" ou implique afiliação com AndroidClaw Labs **não é autorizado e não é afiliado a este projeto**. Forks não autorizados conhecidos serão listados em [TRADEMARK.md](docs/maintainers/trademark.md).

Se encontrar falsificação ou uso indevido de marca, por favor [abra um issue](https://github.com/darkansem12-rgb/zeroclaw-android/issues).

---

## Licença

O AndroidClaw tem licença dupla para máxima abertura e proteção dos contribuidores:
...
O nome e logo do **AndroidClaw** são marcas registradas da AndroidClaw Labs. Esta licença não concede permissão para usá-los para implicar endosso ou afiliação. Veja [TRADEMARK.md](docs/maintainers/trademark.md) para usos permitidos e proibidos.
...
**AndroidClaw** — Zero overhead. Zero compromisso. Implante em qualquer lugar. Troque qualquer coisa. 🦀
