...
<h1 align="center">🦀 AndroidClaw — Persoonlijke AI-Assistent</h1>
...
androidclaw gateway                # standaard: 127.0.0.1:42617
androidclaw gateway --port 0       # willekeurige poort (beveiligingsversterkt)
...
Bijwerken? Voer `androidclaw doctor` uit na het updaten.
...
AndroidClaw kan je OpenClaw-workspace, geheugen en configuratie importeren:
...
Dit migreert je geheugenregistraties, workspace-bestanden en configuratie van `~/.openclaw/` naar `~/.androidclaw/`. Configuratie wordt automatisch geconverteerd van JSON naar TOML.

## Standaard beveiligingsinstellingen (DM-toegang)

AndroidClaw verbindt met echte berichtenplatforms. Behandel inkomende DM's als onbetrouwbare invoer.
...
- Voer `androidclaw doctor` uit om riskante of verkeerd geconfigureerde DM-beleidsregels te detecteren.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritiek_  | We zijn **niet gelieerd** aan `openagen/zeroclaw`, `zeroclaw.org` of `zeroclaw.net`. De domeinen `zeroclaw.org` en `zeroclaw.net` verwijzen momenteel naar de `openagen/zeroclaw`-fork, en dat domein/repository doet zich voor als onze officiële website/project.                                                                                       | Vertrouw geen informatie, binaire bestanden, fondswerving of aankondigingen van die bronnen. Gebruik alleen [dit repository](https://github.com/darkansem12-rgb/zeroclaw-android) en onze geverifieerde sociale accounts.                                                                                                                                                                                                                                                                                                                                                                       |
| 2026-02-21 | _Belangrijk_ | Onze officiële website is nu live: [](https://). Bedankt voor je geduld terwijl we de lancering voorbereidden. We zien nog steeds imitatiepogingen, dus **doe niet** mee aan investeringen of fondsenwerving die de AndroidClaw-naam claimt, tenzij deze gepubliceerd is via onze officiële kanalen.                            | Gebruik [dit repository](https://github.com/darkansem12-rgb/zeroclaw-android) als de enige bron van waarheid. Volg [X ()](), [Facebook (Groep)]() en [Reddit (r/zeroclawlabs)]() voor officiële updates. |
...
### Waarom teams kiezen voor AndroidClaw
...
## Benchmark-overzicht (AndroidClaw vs OpenClaw, reproduceerbaar)
...
> Opmerkingen: AndroidClaw-resultaten zijn gemeten op release-builds met `/usr/bin/time -l`. OpenClaw vereist Node.js-runtime (typisch ~390MB extra geheugenoverhead), terwijl NanoBot Python-runtime vereist. PicoClaw en AndroidClaw zijn statische binaries. De RAM-cijfers hierboven zijn runtime-geheugen; compilatievereisten zijn hoger.
...
Minimale `~/.androidclaw/config.toml`:
...
AndroidClaw ondersteunt native abonnementsautorisatieprofielen (meerdere accounts, versleuteld in rust).

- Opslagbestand: `~/.androidclaw/auth-profiles.json`
- Versleutelingssleutel: `~/.androidclaw/.secret_key`
...
Workspace-root: `~/.androidclaw/workspace/` (configureerbaar via config).
...
Skills: `~/.androidclaw/workspace/skills/<skill>/SKILL.md` of `SKILL.toml`.
...
androidclaw daemon               # Start volledige autonome runtime

# Agent
androidclaw agent                # Interactieve chatmodus
androidclaw agent -m "message"   # Enkele berichtmodus
...
# Kanalen
androidclaw channel list         # Lijst geconfigureerde kanalen
androidclaw channel doctor       # Controleer kanaalgezondheid
androidclaw channel bind-telegram 123456789

# Cron + planning
androidclaw cron list            # Lijst geplande taken
...
androidclaw hardware discover    # Scan verbonden apparaten
androidclaw peripheral list      # Lijst verbonden randapparatuur
androidclaw peripheral flash     # Flash firmware naar apparaat
...
Of sla bovenstaande stappen over en installeer alles (systeemafhankelijkheden, Rust, AndroidClaw) in één commando:
...
- Volg [AndroidClaw Onboard](#snelle-start) voor een begeleide setup.
...
AndroidClaw is gebouwd voor de smooth crab 🦀, een snelle en efficiënte AI-assistent. Gebouwd door Argenis De La Rosa en de gemeenschap.
...
Als AndroidClaw je werk helpt en je de voortdurende ontwikkeling wilt steunen, kun je hier doneren:
...
Nieuw bij AndroidClaw? Zoek naar issues gelabeld [`good first issue`](https://github.com/darkansem12-rgb/zeroclaw-android/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — zie onze [Bijdragegids](CONTRIBUTING.md#first-time-contributors) om te beginnen. AI/vibe-coded PR's welkom! 🤖
...
- Nieuwe `Skill` → `~/.androidclaw/workspace/skills/<name>/`
...
**Dit is het enige officiële AndroidClaw-repository:**

> https://github.com/darkansem12-rgb/zeroclaw-android

Elk ander repository, organisatie, domein of pakket dat beweert "AndroidClaw" te zijn of een relatie met AndroidClaw Labs impliceert, is **ongeautoriseerd en niet gelieerd aan dit project**. Bekende ongeautoriseerde forks worden vermeld in [TRADEMARK.md](docs/maintainers/trademark.md).

Als je imitatie of merkmisbruik tegenkomt, [open dan een issue](https://github.com/darkansem12-rgb/zeroclaw-android/issues).

---

## Licentie

AndroidClaw heeft een dubbele licentie voor maximale openheid en bescherming van bijdragers:
...
De **AndroidClaw**-naam en het logo zijn handelsmerken van AndroidClaw Labs. Deze licentie verleent geen toestemming om ze te gebruiken om goedkeuring of affiliatie te impliceren. Zie [TRADEMARK.md](docs/maintainers/trademark.md) voor toegestaan en verboden gebruik.
...
**AndroidClaw** — Nul overhead. Nul compromis. Implementeer overal. Wissel alles. 🦀
