...
<h1 align="center">🦀 AndroidClaw — مساعد الذكاء الاصطناعي الشخصي</h1>
...
هل تقوم بالترقية؟ شغّل `androidclaw doctor` بعد التحديث.
...
يمكن لـ AndroidClaw استيراد مساحة عمل OpenClaw والذاكرة والتكوين الخاص بك:
...
يقوم هذا بترحيل إدخالات الذاكرة وملفات مساحة العمل والتكوين من `~/.openclaw/` إلى `~/.androidclaw/`. يتم تحويل التكوين من JSON إلى TOML تلقائيًا.

## إعدادات الأمان الافتراضية (الوصول عبر الرسائل المباشرة)

يتصل AndroidClaw بأسطح المراسلة الحقيقية. تعامل مع الرسائل المباشرة الواردة كمدخلات غير موثوقة.
...
- شغّل `androidclaw doctor` لكشف سياسات الرسائل المباشرة الخطرة أو المُعدة خطأ.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _حرج_ | نحن **غير مرتبطين** بـ `openagen/zeroclaw` أو `zeroclaw.org` أو `zeroclaw.net`. نطاقا `zeroclaw.org` و`zeroclaw.net` يشيران حاليًا إلى نسخة `openagen/zeroclaw` المتفرعة، وهذا النطاق/المستودع ينتحل صفة موقعنا/مشروعنا الرسمي. | لا تثق بالمعلومات أو الملفات الثنائية أو جمع التبرعات أو الإعلانات من تلك المصادر. استخدم فقط [هذا المستودع](https://github.com/darkansem12-rgb/zeroclaw-android) وحساباتنا الاجتماعية الموثقة. |
| 2026-02-21 | _مهم_ | موقعنا الرسمي متاح الآن: [](https://). شكرًا لصبركم أثناء تحضيرنا للإطلاق. ما زلنا نرى محاولات انتحال، لذا **لا** تنضم إلى أي نشاط استثمار أو جمع تبرعات يدعي اسم AndroidClaw ما لم يتم نشره عبر قنواتنا الرسمية. | استخدم [هذا المستودع](https://github.com/darkansem12-rgb/zeroclaw-android) كمصدر الحقيقة الوحيد. تابع [X ()]() و[Facebook (Group)]() و[Reddit (r/zeroclawlabs)]() للتحديثات الرسمية. |
...
## لقطة المقارنة المرجعية (AndroidClaw مقابل OpenClaw، قابلة للتكرار)
...
> ملاحظات: نتائج AndroidClaw تم قياسها على إصدارات الإنتاج باستخدام `/usr/bin/time -l`. يتطلب OpenClaw بيئة تشغيل Node.js (عادةً ~390 ميجابايت حمل ذاكرة إضافي)، بينما يتطلب NanoBot بيئة تشغيل Python. PicoClaw وAndroidClaw ملفات ثنائية ثابتة. أرقام الرام أعلاه هي ذاكرة وقت التشغيل؛ متطلبات التجميع في وقت البناء أعلى.
...
الحد الأدنى `~/.androidclaw/config.toml`:
...
يدعم AndroidClaw ملفات تعريف مصادقة أصلية للاشتراك (متعددة الحسابات، مشفرة عند الراحة).

- ملف التخزين: `~/.androidclaw/auth-profiles.json`
- مفتاح التشفير: `~/.androidclaw/.secret_key`
...
جذر مساحة العمل: `~/.androidclaw/workspace/` (قابل للتكوين عبر التكوين).
...
المهارات: `~/.androidclaw/workspace/skills/<skill>/SKILL.md` أو `SKILL.toml`.
...
أو تخطى الخطوات أعلاه وثبّت كل شيء (تبعيات النظام، Rust، AndroidClaw) بأمر واحد:
...
- اتبع [AndroidClaw Onboard](#البداية-السريعة) للإعداد الموجه.
...
تم بناء AndroidClaw للسلطعون الناعم 🦀، مساعد ذكاء اصطناعي سريع وفعال. بناه Argenis De La Rosa والمجتمع.
...
إذا ساعدك AndroidClaw في عملك وتريد دعم التطوير المستمر، يمكنك التبرع هنا:
...
جديد على AndroidClaw؟ ابحث عن المشكلات المصنفة [`good first issue`](https://github.com/darkansem12-rgb/zeroclaw-android/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — انظر [دليل المساهمة](CONTRIBUTING.md#first-time-contributors) لمعرفة كيفية البدء. مرحبًا بمساهمات AI/vibe-coded! 🤖
...
- `Skill` جديد → `~/.androidclaw/workspace/skills/<name>/`
...
**هذا هو مستودع AndroidClaw الرسمي الوحيد:**

> https://github.com/darkansem12-rgb/zeroclaw-android

أي مستودع أو منظمة أو نطاق أو حزمة أخرى تدعي أنها "AndroidClaw" أو تشير إلى انتمائها لـ AndroidClaw Labs هي **غير مصرح بها وغير مرتبطة بهذا المشروع**. سيتم سرد النسخ المتفرعة غير المصرح بها المعروفة في [TRADEMARK.md](docs/maintainers/trademark.md).

إذا واجهت انتحالًا أو إساءة استخدام للعلامة التجارية، يرجى [فتح مشكلة](https://github.com/darkansem12-rgb/zeroclaw-android/issues).

---

## الترخيص

AndroidClaw مرخص بترخيص مزدوج لأقصى انفتاح وحماية للمساهمين:
...
اسم وشعار **AndroidClaw** هما علامتان تجاريتان لـ AndroidClaw Labs. لا يمنح هذا الترخيص إذنًا لاستخدامهما للإشارة إلى التأييد أو الانتماء. انظر [TRADEMARK.md](docs/maintainers/trademark.md) للاستخدامات المسموحة والمحظورة.
...
**AndroidClaw** — صفر حمل زائد. صفر تنازلات. انشر في أي مكان. بدّل أي شيء. 🦀
