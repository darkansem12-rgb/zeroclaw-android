...
<h1 align="center">🦀 AndroidClaw — Персональный ИИ-ассистент</h1>
...
`androidclaw onboard` запускается автоматически после установки для настройки рабочего пространства и провайдера.
...
Обновляетесь? Выполните `androidclaw doctor` после обновления.
...
AndroidClaw может импортировать ваше рабочее пространство, память и конфигурацию OpenClaw:
...
Это переносит ваши записи памяти, файлы рабочего пространства и конфигурацию из `~/.openclaw/` в `~/.androidclaw/`. Конфигурация автоматически конвертируется из JSON в TOML.

## Настройки безопасности по умолчанию (доступ через ЛС)

AndroidClaw подключается к реальным поверхностям обмена сообщениями. Относитесь к входящим ЛС как к ненадёжному вводу.
...
- Выполните `androidclaw doctor` для выявления рискованных или неправильно настроенных политик ЛС.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Критический_ | Мы **не связаны** с `openagen/zeroclaw`, `zeroclaw.org` или `zeroclaw.net`. Домены `zeroclaw.org` и `zeroclaw.net` в настоящее время указывают на форк `openagen/zeroclaw`, и этот домен/репозиторий выдают себя за наш официальный сайт/проект. | Не доверяйте информации, бинарным файлам, сбору средств или объявлениям из этих источников. Используйте только [этот репозиторий](https://github.com/darkansem12-rgb/zeroclaw-android) и наши верифицированные аккаунты в социальных сетях. |
| 2026-02-21 | _Важный_ | Наш официальный сайт теперь доступен: [](https://). Спасибо за терпение, пока мы готовили запуск. Мы по-прежнему видим попытки имитации, поэтому **не** присоединяйтесь к каким-либо инвестиционным или фандрайзинговым активностям, использующим имя AndroidClaw, если они не опубликованы через наши официальные каналы. | Используйте [этот репозиторий](https://github.com/darkansem12-rgb/zeroclaw-android) как единственный источник истины. Следите за обновлениями в [X ()](), [Facebook (Group)]() и [Reddit (r/zeroclawlabs)](). |
...
### Почему команды выбирают AndroidClaw
...
## Снимок бенчмарков (AndroidClaw vs OpenClaw, воспроизводимый)
...
> Примечания: результаты AndroidClaw измерены на релизных сборках с использованием `/usr/bin/time -l`. OpenClaw требует среду выполнения Node.js (обычно ~390 МБ дополнительных накладных расходов памяти), а NanoBot требует среду выполнения Python. PicoClaw и AndroidClaw — статические бинарные файлы. Показатели ОЗУ выше — это память времени выполнения; требования к компиляции при сборке выше.
...
Минимальный `~/.androidclaw/config.toml`:
...
AndroidClaw поддерживает нативные профили аутентификации по подписке (мультиаккаунт, шифрование в состоянии покоя).

- Файл хранилища: `~/.androidclaw/auth-profiles.json`
- Ключ шифрования: `~/.androidclaw/.secret_key`
...
Корень рабочего пространства: `~/.androidclaw/workspace/` (настраивается через конфигурацию).
...
Навыки: `~/.androidclaw/workspace/skills/<skill>/SKILL.md` или `SKILL.toml`.
...
Или пропустите шаги выше и установите всё (системные зависимости, Rust, AndroidClaw) одной командой:
...
- Следуйте [AndroidClaw Onboard](#быстрый-старт) для управляемой настройки.
...
AndroidClaw был создан для smooth crab 🦀 — быстрого и эффективного ИИ-ассистента. Создан Argenis De La Rosa и сообществом.

- [](https://)
- [](https://x.com/zeroclawlabs)

## Поддержите AndroidClaw

Если AndroidClaw помогает вашей работе и вы хотите поддержать дальнейшую разработку, вы можете пожертвовать здесь:
...
Новичок в AndroidClaw? Ищите задачи с меткой [`good first issue`](https://github.com/darkansem12-rgb/zeroclaw-android/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — см. наше [Руководство по участию](CONTRIBUTING.md#first-time-contributors) для начала. AI/vibe-coded PR приветствуются! 🤖
...
- Новый `Skill` → `~/.androidclaw/workspace/skills/<name>/`
...
**Это единственный официальный репозиторий AndroidClaw:**

> https://github.com/darkansem12-rgb/zeroclaw-android

Любой другой репозиторий, организация, домен или пакет, претендующий на звание «AndroidClaw» или подразумевающий связь с AndroidClaw Labs, является **неавторизованным и не связанным с этим проектом**. Известные неавторизованные форки будут перечислены в [TRADEMARK.md](docs/maintainers/trademark.md).

Если вы столкнётесь с имитацией или неправомерным использованием товарного знака, пожалуйста, [откройте issue](https://github.com/darkansem12-rgb/zeroclaw-android/issues).

---

## Лицензия

AndroidClaw распространяется под двойной лицензией для максимальной открытости и защиты участников:
...
Название и логотип **AndroidClaw** являются товарными знаками AndroidClaw Labs. Эта лицензия не предоставляет разрешения на их использование для подразумевания одобрения или принадлежности. См. [TRADEMARK.md](docs/maintainers/trademark.md) для разрешённых и запрещённых использований.
...
**AndroidClaw** — Нулевые накладные расходы. Нулевые компромиссы. Развёртывайте где угодно. Заменяйте что угодно. 🦀
